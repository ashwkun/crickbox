        ...
        <div className="upcoming-detail">
            ...
            {activeTab === 'insights' && h2hData && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {/* H2H Record Card */}
                    {(h2hData as any).team?.head_to_head?.comp_type?.data && (
                        <div style={{
                            background: 'var(--bg-card)',
                            borderRadius: 12,
                            padding: 16,
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                {((h2hData as any).team.head_to_head.comp_type.data as any[]).slice(0, 2).map((t: any, idx: number) => (
                    ...
                    {/* Recent H2H Matches */}
                    {(h2hData as any).team?.against_last_n_matches?.result?.length > 0 && (
                        <div style={{
                            background: 'var(--bg-card)',
                            borderRadius: 12,
                            padding: 16,
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                {((h2hData as any).team.against_last_n_matches.result as any[]).slice(0, 5).map((m: any, idx: number) => (
                    ...
                    {/* Venue Stats */}
                    {(h2hData as any).team?.head_to_head?.venue && (
                        <div style={{
                            background: 'var(--bg-card)',
                            borderRadius: 12,
                            padding: 16,
                            ...
                            <div style={{ fontSize: 13, color: '#fff', fontWeight: 600, marginBottom: 8 }}>{(h2hData as any).team.head_to_head.venue.venue_display_name || (h2hData as any).team.head_to_head.venue.venue}</div>
                            {(h2hData as any).team.head_to_head.venue.data && (
                                <div style={{ display: 'flex', gap: 12 }}>
                                    {((h2hData as any).team.head_to_head.venue.data as any[]).slice(0, 2).map((t: any, idx: number) => (
