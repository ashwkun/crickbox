...
const LiveDetail: React.FC<LiveDetailProps> = ({ match, scorecard, wallstream, onClose, onSeriesClick }) => {
    ...
    // Helper: Get boundary stats from scorecard for current inning
    ...
    const currentStatus = scorecard?.Equation || match.event_sub_status || match.event_status || 'Live';
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, gap: 10 }}>
                        ...
                        <div style={{ textAlign: 'center', minWidth: 60 }}>
                            <div style={{
                                ...
                                background: (() => {
                                    // consistently use the last ball of "this over" if available
                                    const lastBallVal = latestBall?.thisOver?.length > 0
                                        ? latestBall.thisOver[latestBall.thisOver.length - 1]
                        ...
                        <div style={{ flex: 1, textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2 }}>
                                ...
                                {(() => {
                                    const stats = getBoundaryStats(latestBall?.nonStrikerName);
                                    if (stats) return (
                                        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginLeft: 6 }}>
                                            {stats.fours}x4 {stats.sixes}x6
                                        </span>
